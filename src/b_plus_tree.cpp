...
void BPlusTree::split_child(B_Plus_Tree_Node *parent, int index) {
  ...
  new_child->keys.assign(child->keys.begin() + (MAX_B_PLUS_TREE_KEYS / 2),
                         child->keys.end());
  child->keys.resize(MAX_B_PLUS_TREE_KEYS / 2);

  if (!child->is_leaf) {
    new_child->children.assign(child->children.begin() +
                                   (MAX_B_PLUS_TREE_KEYS / 2),
                               child->children.end());
  ...
  parent->children.insert(parent->children.begin() + parent_insert_pos + 1,
                          new_child);
}

BPlusTree::BPlusTree() { root = create_leaf_node(); }
...
void BPlusTree::print_tree() { print_tree_fn(root); }
