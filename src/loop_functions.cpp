...
bool fn_join_loop(LoopFnArgs args)
{
    ...
    // build table input tuple is the tuple that is being compared with the probe table
    // build table is the smaller table
    // probe table is the larger table
    ...
    int ret = RELCAT.getTableEntry(*(args.joinArgs.buildTableName), buildTableEntry);
    ...
    for (int probing_core = 1; probing_core <= staticVar.getNumberOfCores(); probing_core++)
    {
        ReadTupleStream *build_table_input_ts = new ReadTupleStream(buildTableEntry, probing_core);
        ...
        while (true)
        {
            ...
            Tuple tup;
            void *build_input_tuple_val = tup.getTupleValue(args.joinArgs.buildTableAttr, build_input_tuple);
            void *probe_input_tuple_val = tup.getTupleValue(args.joinArgs.probeTableAttr, probe_input_tuple);
            ...
            if (matched)
            {
                ...
                for (const auto &attr1 : output_ts->getAttributeList())
                {
                    ...
                    for (const auto &attr2 : probe_table_ts->getAttributeList())
                    {
                        ...
                        {
                            hit  = true;
                            memcpy((char *)output_tuple + attr1.offset , (char *)probe_input_tuple + attr2.offset, attr1.size);
                        }
                    }
                    ...
                    if(hit == false)
                    {
                        for (const auto &attr2 : build_table_input_ts->getAttributeList())
                        {
                           ...
                           {
                               hit  = true;
                               memcpy((char *)output_tuple + attr1.offset , (char *)build_input_tuple + attr2.offset, attr1.size);
                           }
