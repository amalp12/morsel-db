...
bool Operator::match(void *value1, void *value2, int operatorType,
                     int valueType, int entrySize) {
  switch (valueType) {
  case INTEGER:
...
int Operator::scatter(void *tuple, Morsel *morsel,
                      std::list<Attribute> attributeList) {
  return morsel->insertEntry(tuple, attributeList);
}

void Operator::loop(std::function<bool(union LoopFnArgs args)> fn,
...
// void *  Operator::filter(void * tuple, TupleStream * ts, std::list<Attribute>
// selectList, Morsel * outputMorsel){
